...
| Apex | `AccountPlanController` | `getMatrix(planId)` — segment-aware columns + cells (cacheable). |
| LWC | `whitespaceMatrix` | Renders the colour-coded grid on the Account Plan record page. |
| App / tabs | `Account_Planning` + 3 tabs | Navigation. |
...
2. **Seed the demo data** (creates the Latham law-firm plan + Vodafone corporate plan and their cells)
   `sf apex run --file scripts/seed/seed_account_plans.apex --target-org KJDEV`
3. **Drop the matrix onto the page** (one-time, ~30s): open any Account Plan record →
   gear ▸ **Edit Page** ▸ drag **White-Space Matrix** onto the canvas ▸ **Save** ▸ **Activate**
   (set as Org Default for `Account_Plan__c`).
...
- Objectives, stakeholders, signals, snapshots (§2.4–2.6, §9–10) are **not** in this slice.
