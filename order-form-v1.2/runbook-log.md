...
**RESOLVED same day (Kam, 2026-08-13):** GHK is governed by English law; rule = Americas entities (ALM, LLC) -> New York law, all others (LBR, GHK, MBL) -> English law. Draft markers removed; final clause sentences deployed to KJDEV + FULLUAT and verified (ALM/LLC=NY, LBR/GHK/MBL=English). Also resolved: decision 1 (Legal/Notices = optional Contact lookup on Quote, per-quote entry - built as such, layout placement pending), decision 4 (annual fee = Net Total, confirmed), decision 5 (signatory default from Main/commercial contact, rep can override - confirmed). Still open: ALM registration number (renders blank), decision 6 (General Terms URL live). Payment-due live-terms rendering CONFIRMED by Kam 2026-08-13; decision 2 resolved by v1.3.
